...
counter = 0
rivec = np.linspace(1, 5, 8)
svec = np.linspace(0, 1,8)
...
plt.figure()
plt.pcolor(rivec, svec,np.transpose(GR)/a['f'])
plt.colorbar()
plt.clim((0, 0.5))
plt.contour(rivec, svec, np.transpose(delta), levels=[1], colors='k')
##%%
#cm = 'seismic'
#ET = HHF + LSP + VSP
