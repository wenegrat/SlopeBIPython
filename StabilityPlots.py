...
for i in range(0, thetas.shape[0]):
    ...
    ax1.set_ylabel('Growth rate, $\omega_i$', fontsize=fs)
    ax1.set_ylim((0, .25))
...
ax[0].set_xlabel('V [m s$^{-1}$]', fontsize=fs, color='b')
ax2 = ax[0].twiny()
...
for tl in ax[0].get_xticklabels():
    tl.set_color('b')
ax[0].set_ylabel('$\hat{z}$ [m]', fontsize=24)
ax2.grid(linestyle='--', alpha=0.5)
ax[0].grid(linestyle='--', alpha=0.5)
ax2.set_xlabel('B [m s$^{-2}$]', fontsize=fs, color='g', labelpad=8)
ax[1].semilogx(Ri, z)
...
#plt.tight_layout()
#plt.savefig('/home/jacob/Dropbox/Slope BI/Slope BI Manuscript/RiStructure.eps', format='eps', dpi=1000, bbox_inches='tight')
#plt.savefig('/home/jacob/Dropbox/Slope BI/Slope BI Manuscript/Revision 1/RiStructure.pdf',  bbox_inches='tight')
