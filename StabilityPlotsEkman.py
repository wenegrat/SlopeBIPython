...
for file in sorted(os.listdir(directory)):
    ...
    if filename.endswith(".npz"): 
        ...
        trans[counter] = integrate.trapz(a['U'], a['z'])
#        plt.semilogx(a['ll'], a['gr'])
#        plt.plot(a['ll']*np.sqrt(1.225e-5)*a['H']/a['f'], a['gr']/(a['f']))
        plt.plot(a['ll']*np.sqrt(1.225e-5)*a['H']/a['f'], a['gr']*np.sqrt(a['Bz'][20])/(a['f']*a['Vz'][20]))
...
time= time[idx]
trans = trans[idx]
dtransdt = np.gradient(trans, 6*3600)
ttheory = -trans/dtransdt
wave = wave[idx]*a['N']*a['H']/a['f']
#plt.legend(thetas)
        #%%%
plt.figure()
fig, ax = plt.subplots(2, 1, sharex=True, figsize=(12, 6))
ax[0].plot(time*a['f']/(2*np.pi), grt/a['f'], marker="x", linestyle='None')
ax[0].plot(time*a['f']/(2*np.pi), 2*pi/(ttheory)*1/a['f'], marker="+", linestyle='None')
ax[0].set_ylim(0, 0.3)
